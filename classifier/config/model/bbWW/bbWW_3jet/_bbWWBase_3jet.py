...
class bbWW3jetTrain(KFoldTrain):
    model: str
    ...
    argparser.add_argument(
        ...
        type=parse.mapping,
        default="",
        help=f"bbWW_3jet architecture {parse.EMBED}",
    )
    argparser.add_argument(
        "--ghost-batch",
        type=parse.mapping,
    ...
    def initializer(self, splitter: Splitter, **kwargs):
        ...
        arch = bbWW3jetArch(**({"loss": self.loss} | self.opts.architecture))
...
class bbWW3jetEval(KFoldEval):
    model: str
    output_definition: Callable[[BatchType], BatchType]

    def initializer(self, model, splitter, **kwargs):
        from bbreww.classifier.ml.models.bbWW_3jet import bbWW3jetEvaluation

        return bbWW3jetEvaluation(
            saved_model=model,
            cross_validation=splitter,
