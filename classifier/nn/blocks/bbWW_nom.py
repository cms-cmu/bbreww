...
class InputEmbed(nn.Module):
    def __init__(
        ...
        # non-b jet embedder: (pt, eta, phi, mass) + appended label row -> 5 channels.
        # bbWW_nom has no ml_jet_score / attn_score input; the 4 kinematic
        # features get a padded-jet label row appended in dataPrep (-> 5).
        ...
        # Regressed neutrino 4-vector (px, py, pz, E) embedder
        ...
        # bbWW_nom: 2 b-jets, 2 non-b jets -> C(2,2) = 1 dijet candidate
        self.bsl, self.wsl = 2, 2
        self.qqsl = self.wsl * (self.wsl - 1) // 2  # C(2,2) = 1
    ...
    def dataPrep(self, b, nb, l, a, reg_nu=None):
        ...
        n = b.shape[0]
        b = b.view(n, 5, 2)
        nb = nb.view(n, 4, -1)               # (n, 4, wsl): pt, eta, phi, mass
        l = l.view(n, 6, 1)
        a = a.view(n, self.dA, 1)

        # bbWW_nom has no ml_jet_score / attn_score input branch: non-b jets carry
        # only the 4 kinematic features (pt, eta, phi, mass).

        # Save raw kinematics before any transforms (needed for hadW_mass / pair mjj)
        raw_nb = nb.reshape(n, -1).clone()     # (n, 4*wsl) flat
        raw_lep = l.squeeze(-1).clone()        # (n, 6)

        # log(HT): look up HT's position dynamically since the ancillary feature
        # list is configured per-workflow.
        ...
        # C(2,2) = 1 dijet candidate from 2 non-b jets
        qq, qqPxPyPzE = addFourVectors(
            nb[:, :, (0,)], nb[:, :, (1,)]
        )

        ## Hadronic top: 2 b-jets x qqsl dijet candidates = 2*1 = 2 candidates
    ...
    def updateMeanStd(self, b, nb, l, a, reg_nu=None):
        (b, bb, qq, a, nb, l, lnu_mT, bWhad, bWlep, lepQQdR,
         ...
         mask, mask_bbMdR, mask_qqMdR, mask_bbn, mask_qq, mask_bWhad, mask_bWlep,
         derived_kinematics, raw_nb, raw_lep,
         reg_nu, regW_lep, lepW_mass) = self.dataPrep(
            b, nb, l, a, reg_nu)
    ...
    def forward(self, b, nb, l, a, reg_nu=None):
        ...
        # Regressed neutrino embedding (4-vector: px, py, pz, E)
...
class bbWW_nom(nn.Module):
    def __init__(
        ...
        device="cuda",
        nClasses=1,
        architecture="bbWW_nom",
    ):
        super(bbWW_nom, self).__init__()
        self.debug = False
        self.dA = len(ancillaryFeatures)
        self.dD = dijetFeatures
        self.device = device
        ...
        self.attention_WW = MinimalAttention(
            ...
            scalar_dim=qqsl + 1,  # qqsl lepQQdR + 1 lnu_mT
            layers=self.layers,
        ...
        # All qqsl qq pairs used -> bsl*qqsl hadronic x bsl leptonic pairings
        ...
        # bbWW_nom has no ml_jet_score input, so there is no external attn-score
        # prior (pair_attn); qv_combine_pair combines only mjj + dRjj + dRlepqq.
        ...
        self.qv_combine_pair = GhostBatchNorm1d(
            3 * self.dD, features_out=self.dD, conv=True,
            name="pair qv combine mjj+dRjj+dRlepqq"
        )

        # Fixed pair indices for C(2,2) = 1 qq pair: (0,1)
        self.register_buffer(
            "_pair_i", torch.tensor([0], dtype=torch.long), persistent=False
        )
        self.register_buffer(
            "_pair_j", torch.tensor([1], dtype=torch.long), persistent=False
        )
        ...
        for _p, (_i, _j) in enumerate(zip([0], [1])):
    ...
    def forward(self, b, nb, l, a, reg_nu=None):
        ...
        (b, bb, qq, a, nb, l, lnu_mT, bWhad, bWlep, lepQQdR,
         ...
         mask_bbn, mask_qq, mask_bWhad, mask_bWlep,
         derived, reg_nu_emb, regW_emb, lepW_mass_emb,
         raw_nb, raw_lep) = self.inputEmbed(
            b, nb, l, a, reg_nu
        )

        n = b.shape[0]
        wsl = self.inputEmbed.wsl     # 2
        qqsl = self.inputEmbed.qqsl   # 1
        ...
        # ============================================================
        # TT attention: all qqsl qq pairs, bsl*qqsl hadronic x bsl leptonic
        # ============================================================
        ...
        bWlep_exp = bWlepMdR.squeeze(-1).repeat(1, 1, n_bWhad)  # (n, dD, n_tt)

        # Map each qq pair to its constituent non-b jets for bbn relationship
        bbn_flat = bbnMdR.squeeze(2)  # (n, dD, wsl)
        qq_idx = []
        for i in range(wsl):
        ...
        bbn_exp = bbn_qq.repeat_interleave(n_bWlep, dim=2).repeat(1, 1, bsl)  # (n, 2*dD, n_tt)

        bbqq_exp = bbqqMdR.squeeze(2)  # (n, dD, qqsl)
        bbqq_exp = bbqq_exp.repeat_interleave(n_bWlep, dim=2).repeat(1, 1, bsl)  # (n, dD, n_tt)
        ...
        # ============================================================
        # WW attention: regressed leptonic W queries the qq dijet hypotheses.
        ...
        # ============================================================
        qq_pairs = qq[:, :, :qqsl]                         # (n, dD, qqsl) dijet values
        pair_mask = mask_qq.view(n, 1, qqsl)               # (n, 1, qqsl) pair padded iff any jet padded

        # Per-pair raw physics quantities (from un-log-transformed jet 4-vectors)
        raw_nb_4 = raw_nb.view(n, 4, -1)[:, :, :wsl]       # (n, 4, wsl)
        j1_raw = raw_nb_4[:, :, self._pair_i]              # (n, 4, qqsl)
        j2_raw = raw_nb_4[:, :, self._pair_j]              # (n, 4, qqsl)
        pair_mjj  = diObjectMass(PxPyPzE(j1_raw), PxPyPzE(j2_raw))  # (n, 1, qqsl)
        pair_dRjj = calcDeltaR(j1_raw, j2_raw)                       # (n, 1, qqsl)

        # Per-pair angular separation from the lepton (already computed in dataPrep)
        pair_dRlepqq = lepQQdR.view(n, 1, qqsl)                      # (n, 1, qqsl)
        ...
        pair_qv = self.qv_combine_pair(
            torch.cat([pair_mjj_e, pair_dRjj_e, pair_dRlepqq_e], dim=1),
            pair_mask,
        )                                                   # (n, dD, qqsl)

        WW, WW0, WW_weights = self.attention_WW(
            regW_emb,               # q: (n, dD, 1) regressed leptonic W
            qq_pairs,               # v: (n, dD, qqsl) dijet hypotheses
            pair_mask,              # mask: (n, 1, qqsl)
            regW0,                  # q0: residual
            pair_qv,                # per-pair physics bias
            scalars,                # qqsl lepQQdR + 1 lnu_mT
            self.debug,
        )
        ...
        pair_w = WW_weights.squeeze(2).mean(dim=1)                   # (n, qqsl)
        ...
        jet_marg = torch.matmul(pair_w, self._jet_pair_incidence.t())  # (n, wsl)
        ...
        HH = torch.cat([
            ...
            bbnMdR.squeeze(2),            # (n, dD, wsl)
            qqMdR.view(n, self.dD, -1),   # (n, dD, wsl*wsl)
            scalars_emb,                  # (n, dD, 1)
            derived,                      # (n, dD, 1) -- derived kinematics
            bbWlepMdR.squeeze(2),         # (n, dD, 1) -- bb vs regW MdR
            WlepNBMdR.squeeze(2),         # (n, dD, wsl) -- regW vs non-b jets MdR
        ], dim=-1)
        HH_final = self.HH_final_embed(HH)

        HH_logits_pre = torch.cat([HH_final, TT_sel], dim=-1)
        pooled = self.out(HH_logits_pre)                         # (n, 16)

        # Direct (non-pooled) joint-mass feature: lets the classifier read the
        # (m_hadW, m_lepW) correlation without averaging with other positions.
